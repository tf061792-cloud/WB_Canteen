...
// 通用请求函数（带缓存�?async function request(url, options = {}, cacheOptions = {}) {
  const token = getToken();
  const { useCache = false, cacheTtl = 30000 } = cacheOptions;

  // 检查缓�?  if (useCache && (!options.method || options.method === 'GET')) {
    const cached = requestCache.get(url, options);
    if (cached) return cached;
  }

  // 判断是否�?FormData 请求（文件上传）
...
const productAPI = {
  ...
  // 根据商品名称查询商品（用于订单编辑时自动更新价格�?  searchByName: (name) => request(`/products/search/by-name?name=${encodeURIComponent(name)}`)
};
...
// 通用 admin API（用于新模块�?const adminAPI = {
  get: (url) => request(url),
